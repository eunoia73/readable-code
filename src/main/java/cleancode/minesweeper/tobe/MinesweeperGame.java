...
public class MinesweeperGame {
    ...
    private static void initializeGame() {
        ...
        for (int i = 0; i < LAND_MINE_COUNT; i++) {
            ...
            BOARD[row][col].turnOnLandMine();  //지뢰 세팅
        }

        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (isLandMineCell(row, col)) {  //지뢰이면,
                    continue;
                }
    ...
    private static int countNearbyLandMines(int row, int col) {
        int count = 0;
        if (row - 1 >= 0 && col - 1 >= 0 && isLandMineCell(row - 1, col - 1)) {  //왼쪽 위
            count++;
        }
    ...
    private static void open(int row, int col) {
        ...
        if (BOARD[row][col].isOpened()) {  // 이미 열려있는 cell
            return;
        }
        if (isLandMineCell(row, col)) {  // 지뢰가 있는 cell
            return;
        }

        BOARD[row][col].open();

        if (BOARD[row][col].hasLandMineCount()) {  // 주변에 지뢰가 있는 cell
            return;
        }
